...
const Comparison = {
    ...
    init() {
        if (typeof Utils === 'undefined') {
            console.warn('Comparison: Utils not available');
            return;
        }
        this.modal = Utils.getElement('comparisonModal');
        if (!this.modal) {
            console.warn('Comparison: Modal element not found');
            return;
        }
    ...
    setupEventListeners() {
        ...
        if (compareBtn) {
            compareBtn.addEventListener('click', () => {
                if (this.selectedItems.length >= 2) {
                    ...
                    Utils.showToast('Моля изберете поне 2 квартала за сравнение', 'info');
                }
            });
        }
        ...
        // ESC key to close (bound handler for cleanup)
        this.escHandler = (e) => {
    ...
    /**
     * Get item selector string
     * @param {Object} entry - Entry object with city and neighborhood
    ...
    addCompareCheckbox(card, entry) {
        // Only for neighborhoods (not childcare/doctors/shops in initial version)
        const locationType = entry.locationType || 'neighborhood';
        if (locationType !== 'neighborhood') return;
    ...
    addItem(entry) {
        if (this.selectedItems.length >= this.maxItems) {
            Utils.showToast(`Можете да сравните максимум ${this.maxItems} квартала`, 'warning');
            // Uncheck the checkbox
            const checkbox = document.querySelector(this.getItemSelector(entry));
        ...
        if (navigator.share) {
            ...
            if (navigator.clipboard) {
                navigator.clipboard.writeText(fullText).then(() => {
                    Utils.showToast('Линк копиран! Споделете го с приятели.', 'success');
                }).catch(() => {

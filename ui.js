...
function updateNeighborhoodOptions() {
    ...
    if (!select) return; // Guard against null select element
...
function displayResults(cityFilter = '', neighborhoodFilter = '', sortBy = 'rating-desc', minVotes = 0, minRating = 0) {
    ...
    // Filter by city (but not for shops - they show all cities; services show only the selected city)
    ...
    // For services mode, filter by category if specified
    } else if (AppState.getLocationType() === 'services' && neighborhoodFilter) {
        filteredRatings = filteredRatings.filter(r => {
            // Extract category from "Emoji Name (Category) - Neighborhood, City" format
            const match = r.neighborhood.match(/\(([^)]+)\)/);
            const category = match ? match[1] : '';
            // Match by category key or name
            const categoryName = serviceCategories[neighborhoodFilter]?.name || neighborhoodFilter;
            return category === categoryName || category === neighborhoodFilter;
        });
    ...
    const sortedEntries = Object.entries(grouped).map(([neighborhood, neighborhoodRatings]) => {
        ...
        // Also extract category for services
        let specialty = '';
        ...
        if (locationType === 'childcare' || locationType === 'schools' || locationType === 'doctors' || locationType === 'services' || locationType === 'shops') {
            // For childcare, schools, doctors, services and shops: only 'overall' rating
    ...
    // Apply split layout for all pages (results left, form right)
    const mainContent = document.getElementById('main-content');
    if (mainContent && !mainContent.classList.contains('split-layout')) {
        mainContent.classList.add('split-layout');
    }

    // Default: original layout with split CSS
    container.innerHTML = '';
    ...
    if (typeof AdSenseManager !== 'undefined' && typeof debouncedRefreshAds !== 'undefined') {
        const adsContainer = document.getElementById('resultsContainer');
        if (adsContainer && adsContainer.children.length > 0 && adsContainer.textContent.trim().length >= 50) {
            debouncedRefreshAds();
        }
    }
}
...
function renderResultBatch(entries, container) {
    entries.forEach(({ neighborhood, city, neighborhoodRatings, avgRatings, totalAvg, locationType }) => {
        ...
        let ratingGridHTML = '';
        if (locationType === 'childcare' || locationType === 'schools' || locationType === 'doctors' || locationType === 'services' || locationType === 'dentists' || locationType === 'shops') {
            // For childcare, schools, doctors, services, dentists and shops: show only overall rating
            ratingGridHTML = `
        ...
        card.innerHTML = `
            ...
            ${opinionHTML}
            ${typeof SocialSharing !== 'undefined' ? SocialSharing.createShareButtons(neighborhood, totalAvg.toFixed(1), city) : ''}
        `;
        ...
        if (typeof Favorites !== 'undefined') {
            Favorites.addFavoriteButton(card, { neighborhood, city, locationType });
        }

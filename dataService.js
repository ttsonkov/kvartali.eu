...
const DataService = {
    ...
    init() {
        ...
        this._ensureSchoolsNeighborhoods();
    },
    ...
    _ensureSchoolsNeighborhoods() {
        if (typeof schoolsNeighborhoods === 'undefined') {
            console.warn('schoolsNeighborhoods not loaded, using empty fallback');
            window.schoolsNeighborhoods = {};
            Object.keys(cityNeighborhoods).forEach(city => {
                window.schoolsNeighborhoods[city] = [];
            });
        }
    },
    ...
    getNeighborhoodsForCity(city, type = 'neighborhood') {
        ...
        if (!city) return type === 'childcare' || type === 'schools' ? [] : allNeighborhoods;
        let source;
        if (type === 'childcare') {
            source = childcareNeighborhoods;
        } else if (type === 'schools') {
            source = schoolsNeighborhoods;
        } else {
            source = cityNeighborhoods;
        }
        return source[city] || [];
    },

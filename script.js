...
if (ratingFormEl) {
    ratingFormEl.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        const neighborhood = neighborhoodEl ? neighborhoodEl.value : '';
        ...
        try {
            ...
            // Reset form
            const ratingFormReset = document.getElementById('ratingForm');
            if (ratingFormReset) ratingFormReset.reset();
...
if (filterNeighborhoodEl) {
    filterNeighborhoodEl.addEventListener('change', (e) => {
        const city = document.getElementById('filterCity')?.value || currentCity;
        displayResults(city, e.target.value);
    });
}

// Header city link toggles menu
const headerCityLinkEl = document.getElementById('headerCityLink');
if (headerCityLinkEl) {
    headerCityLinkEl.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        toggleHeaderMenu();
    });
...
function updateNeighborhoodOptions() {
    ...
    const citySelect = document.getElementById('citySelect');
    const city = (citySelect ? citySelect.value : '') || currentCity;
...
function populateSelectOptions(formCity = currentCity, filterCity = null) {
    if (filterCity === null) {
        const filterCityEl = document.getElementById('filterCity');
        filterCity = filterCityEl ? filterCityEl.value : currentCity;
    }
